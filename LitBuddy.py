...
def toggle_dyslexic():
    ...
    try:
        tkfont.nametofont("OpenDyslexic")
        chosen_font = "OpenDyslexic"
    except Exception:
        chosen_font = "Comic Sans MS"
        if not hasattr(toggle_dyslexic, "warned"):
            messagebox.showwarning("Font Missing", "OpenDyslexic font not found. Using fallback font.")
            toggle_dyslexic.warned = True
...
button_row = tk.Frame(left_frame, bg="#FFFDF7")
button_row.pack(pady=10)

clear_btn = tk.Button(button_row, text="🗑️ Clear Text", font=(CHOSEN_FONT, 12), bg="#FFC0CB",
                      command=lambda: write_text.delete("1.0", tk.END))
clear_btn.pack(side="left", padx=5)

mic_btn = tk.Button(button_row, text="🎤 Talk to Write", font=(CHOSEN_FONT, 12), bg="#DFF5FF",
                    command=start_voice_input)
mic_btn.pack(side="left", padx=5)

test_voice_btn = tk.Button(button_row, text="🔊 Test Voice", font=(CHOSEN_FONT, 11),
                           command=test_typing_voice)
...
mic_btn.pack(side="left", fill="x", expand=True, padx=5)
test_voice_btn.pack(side="left", fill="x", expand=True, padx=5)
...
def load_next_multiplication_problem():
    ...
    if fixed_number_mode and fixed_number_value is not None:
        # Use fixed number as 'a', generate 'b' based on level
        a = fixed_number_value
        ...
        # Normal random generation based on level
        if level == 1:
...
def build_multiplication_game_tab():
    ...
    def activate_fixed_number_mode_multiplication():
        global fixed_number_mode, fixed_number_value
        fixed = simpledialog.askinteger("🔢 Fixed Number Mode", "Enter a number to fix (e.g. 12):", minvalue=1, maxvalue=999)
        if fixed is None:
            return
        fixed_number_mode = True
        fixed_number_value = fixed
    ...
    def deactivate_fixed_number_mode_multiplication():
        global fixed_number_mode, fixed_number_value
        fixed_number_mode = False
        fixed_number_value = None
        messagebox.showinfo("🔚 Mode Off", "Fixed Number Mode has been turned off.")
    ...
    # 📦 Fixed Number Mode Buttons Frame
    fixed_mode_frame = tk.Frame(multiplication_tab, bg="#FFF3E0")
    fixed_mode_frame.pack(pady=5)

    # 🔢 Activate Fixed Mode
    tk.Button(fixed_mode_frame, text="🔢 Fixed Number Mode", font=(CHOSEN_FONT, 11), bg="#FFF9C4", command=activate_fixed_number_mode_multiplication).pack(side="left", padx=5)

    # ❌ Deactivate Fixed Mode
    tk.Button(fixed_mode_frame, text="❌ Exit Fixed Mode", font=(CHOSEN_FONT, 11), bg="#FFCDD2",
              command=deactivate_fixed_number_mode_multiplication).pack(side="left", padx=5)
